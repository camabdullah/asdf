'use strict';
const RoadProgress=(()=>{
 const fresh=()=>({version:1,layoutVersion:7,current:0,unlocked:0,completed:[],memories:[],seen:[],states:{},minutes:0,volume:.4,assist:false});
 function migrate(raw,levels){
  ...
  const old=raw.layoutVersion||1,v={...base,...raw},map=n=>{let k=Number(n)||0;if(old<5&&k>=12)k++;if(old<6&&k>=13)k++;if(old<7&&k>=9)k+=2;return Math.max(0,Math.min(last,k));};
  ...
  v.layoutVersion=7;return {save:v,migrated:old<7};
 }
