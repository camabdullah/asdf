...
(async()=>{
 ...
 assert.equal(chapters.length,16);assert.deepEqual([...new Set(chapters.filter(Boolean))].sort(),Object.keys(tracks).sort());
...
assert.equal(chapters.length,16);assert.ok(chapters.every(k=>tracks[k]));
